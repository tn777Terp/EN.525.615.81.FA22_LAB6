...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    clk_count = stop_time-start_time;
    ...
    unsigned clk_speed  = 125000000;
    unsigned bytes_transferred = 2048 << 3; // Each read is 32-bits wide
    double   time_spent = clk_count / (double)clk_speed;
    double   throughput = ((double)bytes_transferred / time_spent) / 1000000; 

    printf("You transferred %u bytes of data in %lf seconds\n", bytes_transferred, time_spent);
    printf("Measured Transfer throughput = %.2lf Mbytes/sec\n", throughput);
}
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Tin Nguyen - Custom Peripheral Demonstration\n\r");
    *(my_periph+RADIO_TUNER_CONTROL_REG_OFFSET) = 0; // make sure rWadio isn't in reset
